...
import { resultados } from "./resultado-singleton.js";
...
setInterval(() => {
    if (proximoSegundo >= 0 && proximoSegundo < 10) {
        ...
        resultados.resultadoPorQuestao.push('não respondida')
        montarQuestao();
    }
...
function responderAlternativa1(){
    if (dados[questaoAtual].resposta === 1){
        resultados.resultadoPorQuestao.push('Resposta Correta');
        resultados.qtdQuestoesCorretas++;
        resultados.pontuacao += (100 * proximoSegundo);
    } else {
        resultados.resultadoPorQuestao.push('Resposta Incorreta');
    }
    montarQuestao();
}
